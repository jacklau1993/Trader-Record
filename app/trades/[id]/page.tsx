...
import { getTrades, saveTrade, Trade, getCategories, Category } from "@/lib/storage";
...
export default function TradeDetailPage() {
    ...
    useEffect(() => {
        ...
        if (updates) {
            setTrade(prev => prev ? ({ ...prev, ...updates }) : null);
        }
    }, [trade?.entryPrice, trade?.exitPrice, trade?.stopLoss, trade?.profitTarget, trade?.type]); // Dependencies for recalc
    ...
    const toggleTag = (tagId: string) => {
        if (!trade) return;
        const newTags = trade.tags.includes(tagId)
            ? trade.tags.filter(t => t !== tagId)
            : [...trade.tags, tagId];
