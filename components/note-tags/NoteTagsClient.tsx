...
import { createNoteTagCategory, deleteNoteTagCategory, updateNoteTagCategory, createNoteTag, deleteNoteTag } from "@/app/actions/note-tag-actions";
...
export default function NoteTagsClient({ initialCategories }: { initialCategories: any[] }) {
    ...
    const handleDeleteCategory = async (catId: string) => {
        ...
        try {
            await deleteNoteTagCategory(catId);
            router.refresh();
    ...
    const handleUpdateCategoryColor = async (catId: string, color: string) => {
        ...
        try {
            await updateNoteTagCategory(catId, { color });
            router.refresh();
    ...
    return (
        <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
            ...
            <p className="text-sm text-muted-foreground">
                Note Tags are used to categorize your daily journal entries. They are separate from Trade Tags.
            </p>
            ...
            {categories.length === 0 && (
                <div className="flex flex-col items-center justify-center h-64 text-muted-foreground border border-dashed border-border rounded-lg">
                    ...
                    <p className="text-lg font-medium">No Note Tag Categories</p>
                    <p className="text-sm">Create your first category above to start tagging your notes.</p>
                </div>
            )}
        </div>
    );
