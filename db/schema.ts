...
export const noteTagDefinitions = sqliteTable("note_tag_definition", {
    ...
    name: text("name").notNull(),
    color: text("color"),
    categoryId: text("category_id").notNull().references(() => noteTagCategories.id, { onDelete: "cascade" }),
    userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
    updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date())
});
...
export const noteTagAssignments = sqliteTable("note_tag_assignment", {
    noteId: text("note_id").notNull().references(() => notes.id, { onDelete: "cascade" }),
    tagId: text("tag_id").notNull().references(() => noteTagDefinitions.id, { onDelete: "cascade" }),
});
