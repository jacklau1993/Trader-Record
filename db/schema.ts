...
export const propFirmTransactions = sqliteTable("prop_firm_transaction", {
  id: text("id").primaryKey(),
  ...
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
  type: text("type").notNull(), // "EXPENSE" | "PAYOUT"
  amount: real("amount").notNull(),
  currency: text("currency").notNull(), // "GBP" for expenses, "USD" for payouts
  month: text("month").notNull(), // "2026-01", "2026-02", etc.
  note: text("note"),
  createdAt: integer("created_at", { mode: "timestamp" })
    .notNull()
    .$defaultFn(() => new Date()),
